...
exports.run = (client, message, args) => {
  const kayıtlı = message.guild.roles.find(r => r.id === "789006350524874762"); // ERKEK RÖLÜ
  const kayıtlı3 = message.guild.roles.find(r => r.id === "789006350868676610"); // ERKEK RÖLÜ
  const kayıtlı2 = message.guild.roles.find(r => r.id === "789006351782641674"); // ERKEK RÖLÜ
  ...
  if(!message.member.roles.array().filter(r => r.id === "789006353897095189")[0]) { // KAYIT YETKİLİSİ
    ...
    .addField(`Yeni isim\n`, `${tag} ${nick} , ${yas}`)
    .setFooter("Merolos | Kayıt sistemi")
    .setColor("BLECK")
    log.send(embed)
  }
}
